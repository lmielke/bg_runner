...
def pretty_print_messages(messages, *args, verbose: int = 0, clear=True, save=True, **kwargs):
    ...
    # print(printable)
    if save:
        save_table(tbl, *args, **kwargs)
    return tbl


def to_tbl(
    data, *args, verbose: int = 0, headers=["EXPERT", "MESSAGE"], tablefmt="simple", **kwargs
):
    tbl = []
    for m in data:
...
def hide_tags(text, *args, verbose: int = 0, **kwargs):
    ...
    for start, end in sts.tags.values():
        ...
        text = re.sub(rf"({start}.*?{end})", replacement, text, flags=re.DOTALL)
    return "\n" + text
