    ...
    def finalize_graph(self):
        ...
        for node, count in self.incoming_edges.items():
            ...
            self.graph.node(
                node,
                fontsize=fontsize,
                fillcolor="lightblue" if node == self.main_file else fillcolor,
            )
                ...
                for alias in node.names:
                    ...
                    if module_name.startswith(self.package_name):
                        imports.append(
                            (
                                alias.name,
                                os.path.relpath(filepath, self.root_dir).replace(os.sep, "."),
                            )
                        )
...
def set_params(*args, **kwargs):
    parser = argparse.ArgumentParser(
        description="Analyze Python package structure and visualize import relationships."
    )
    parser.add_argument(
        "main_file_name", type=str, help="The name of the main Python file to trace imports from."
    )
    parser.add_argument(
        "--verbose",
        type=int,
