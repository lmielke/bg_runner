...
from runner.helpers.collections import temp_chdir
...
def tree_to_dirs(tree, *args, **kwargs):
    ...
    |-- ▼ logs
        |...
    |-- ▼ testopia_logs
        |-2024-01-12-14-26-24-796531_test_bgrpackage.log
        |...
    |...
    ...
    ['test\\testopia_logs\\2024-01-12-14-26-24-796531_test_bgrpackage.log', False],
    ...,]
    ...
    def _parse_tree(tree, *args, **kwargs):
        ...
        for i, line in enumerate(_normalize(tree, *args, **kwargs), 0):
            ...
            if not line or line == dir_discontinued_raw:
                continue
...
def pipenv_is_active(exec_path, *args, **kwargs):
    ...
    is_active = os.path.basename(exec_path.split("Scripts")[0].strip(os.sep)).startswith(
        sts.project_name
    )
    return is_active
