...
from runner.helpers.tree import Tree
from runner.helpers.import_info import main as import_info
from runner.helpers.package_info import pipenv_is_active
    ...
    if infos:
        for info in infos:
            try:
                ...
                print(
                    f"{Fore.YELLOW}Warning:{Fore.RESET} {info}_info function not found. Skipping..."
                )
    collect_infos(
        f"{Fore.YELLOW}\nfor more infos: " f"{Style.RESET_ALL}bgr info -i {all_infos} or -v 1-3"
    )
    user_info(*args, **kwargs)


def user_info(*args, **kwargs):
    msg = f"""\n{f" RUNNER USER info ":#^80}"""
...
def package_info(*args, verbose: int = 0, **kwargs):
    ...
    collect_infos(
        f"Project import structure:\n" f"{import_info(main_file_name='runner.py', verbose=0, )}"
    )
