...
from runner.models.system import System
...
class Test_System(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.content = """
                                Certainly! To create your package named `myclocklib` in your temp directory, run the following command in your terminal:  ```shell pipenv run bgr clone -pr 'myclock' -n 'myclock' -a 'clock' -t 'C:\\temp' -p 3.11 --install ```  \nThis will clone `bg_runner` to `C:/Users/lars/myclock`, set up the environment, and install dependencies.
                            """
        cls.code_blocks = """
    ...
    def test_get_code_blocks(self):
        ...
        # print(f"\n{Fore.YELLOW}\nCode blocks: {code_blocks}{Fore.RESET}")

    def test_execute(self):
        system = System()
        codes = [f"pipenv run bgr info", f"dir", "dir /AD", "ls -la"]
        system.commands = [
            {
                "indicator": "```",
                "language": "shell",
    ...
    def test_adhog_assist_shell_action(self):
        ...
        cmds = [
            "dir",
            "bgr clone -pr 'myhammerlib' -n 'myhammer' -a 'myham' -t 'C:/temp' -p '3.10' --install -y",
        ]
